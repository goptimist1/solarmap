    ...
    if gc:
        try:
            ...
            if not df_save.empty:
                values = df_save.fillna("").astype(object).values.tolist()
                ws.update(values=values, range_name=f"A2:{chr(64+len(TARGET_COLS))}{len(values)+1}", value_input_option="USER_ENTERED")
...
def save_target_row(tab_name: str, row_dict: dict, append_if_missing: bool = False) -> bool:
    ...
    if gc:
        try:
            sh = gc.open_by_key(GOOGLE_SHEET_ID)
            ws = get_or_create_ws(sh, tab_name, rows=100, cols=max(len(TARGET_COLS), 20))
            ...
            if not site_id:
                ...
                return False
            try:
                cell = ws.find(site_id)
            except Exception as find_error:
                if GS_AVAILABLE and find_error.__class__.__name__ == "CellNotFound":
                    cell = None
                else:
                    raise
            values = _row_values_from_record(row, headers)
            if cell is not None:
                ...
                ws.update(values=[values], range_name=range_name, value_input_option="USER_ENTERED")
            else:
                if not append_if_missing:
                    st.error(f"Google Sheets에서 {site_id} 행을 찾지 못해 저장을 중단했습니다.")
                    return False
    ...
    try:
        ...
        if site_id in existing["site_id"].astype(str).values:
        ...
        normalize_df(existing).to_csv(filename, index=False, encoding="utf-8-sig")
        load_target_data.clear()
        return True
    except Exception as e:
        st.error(f"CSV 저장 실패: {e}")
...
def append_activity(site_id: str, tab_name: str, before: dict, after: dict, action: str) -> bool:
    ...
    if gc:
        try:
            ...
            st.session_state.activity_rows_session.append(row)
            return True
        except Exception as e:
            st.session_state.activity_status = f"Google Sheets 활동 이력 저장 실패: {e}"
...
def merge_discovered_with_existing(existing: pd.DataFrame, discovered: pd.DataFrame) -> pd.DataFrame:
    ...
    if existing.empty:
        ...
        for i in result.index:
            if not result.at[i, "site_id"]:
                result.at[i, "site_id"] = new_site_id(result.iloc[:i])
    ...
    for _, row in discovered.iterrows():
        ...
        if not key or key in known:
            continue
        row = row.to_dict()
        row["site_id"] = new_site_id(working)
        new_rows.append(row)
        working = pd.concat([working, pd.DataFrame([row])], ignore_index=True)
        known.add(key)
    if new_rows:
        return normalize_df(pd.concat([existing, pd.DataFrame(new_rows)], ignore_index=True))
    return existing
...
<script>
(function(){
  function disableMobileSelectKeyboard(){
    try {
      var selects = document.querySelectorAll('div[data-baseweb="select"] input[role="combobox"]');
  ...
  disableMobileSelectKeyboard();
  setTimeout(disableMobileSelectKeyboard, 100);
  setTimeout(disableMobileSelectKeyboard, 500);
  setTimeout(disableMobileSelectKeyboard, 1500);
  try {
    var observer = new MutationObserver(disableMobileSelectKeyboard);
...
q1, q2 = st.columns(2)
with q1:
    query_clicked = st.button("데이터 조회 및 지도 적용", use_container_width=True, type="primary")
with q2:
    refresh_clicked = st.button("공공데이터 재조회 · 신규 타겟 반영", use_container_width=True)
...
if query_clicked or refresh_clicked:
    ...
    try:
        ...
        if refresh_clicked:
            ...
            save_ok = True
            if existing.empty:
                save_ok = save_target_data(target_tab_name, merged) if not merged.empty else True
            else:
                for _, new_row in added.iterrows():
                    if not save_target_row(target_tab_name, new_row.to_dict(), append_if_missing=True):
                        save_ok = False
                        break
            if save_ok:
                st.session_state.target_data = merged
                st.session_state.last_loaded_tab = target_tab_name
                st.success(f"재조회 완료: 기존 {len(existing):,}건 유지 · 신규 {len(added):,}건 반영")
...
if clicked_marker:
    ...
    if clicked_key:
        ...
        for _, r in df_all.iterrows():
            key = coord_key(r.get("lat"), r.get("lng"))
            if key and key not in coord_map:
                coord_map[key] = str(r.get("site_id", ""))
        sid = coord_map.get(clicked_key, "")
...
if clicked_map and not clicked_marker:
    ...
    if nearest is not None and nearest_dist <= 20.0:
        ...
        st.toast("기존 등록 현장을 불러왔습니다. (20m 이내 기존 핀)")
        st.rerun()

    current = st.session_state.new_pin_coord
    if not current or abs(current["lat"] - lat) > 0.000001 or abs(current["lng"] - lng) > 0.000001:
...
if selected_comp is not None:
    ...
    if st.button("변경사항 저장", use_container_width=True, type="primary", key=f"save_{comp['site_id']}"):
        ...
        if save_target_row(st.session_state.current_tab, after, append_if_missing=False):
            activity_ok = append_activity(comp["site_id"], st.session_state.current_tab, before, after, "수정")
            if not activity_ok:
                st.warning("현장 데이터는 저장되었지만 영업일지 이력은 저장되지 않았습니다.")
    ...
    with st.form("new_pin_form", clear_on_submit=True):
        ...
        if st.form_submit_button("신규 현장 등록", use_container_width=True):
            ...
            if not duplicate.empty:
                ...
                if save_target_row(st.session_state.current_tab, new_row, append_if_missing=True):
                    activity_ok = append_activity(sid, st.session_state.current_tab, {}, new_row, "신규등록")
                    if not activity_ok:
                        st.warning("현장 데이터는 저장되었지만 영업일지 이력은 저장되지 않았습니다.")
                    st.session_state.target_data = updated
                    st.session_state.selected_site_id = sid
